    ...
    for assignment in itertools.product(
        ...
        for val in [True, False]:
            ...
            prob += factor.table.get(
                tuple(full[v] for v in factor.variables),
                0
            )
...
def normalize(factor, query):
    ...
    for assignment, prob in factor.table.items():
        assign_dict = dict(
            zip(factor.variables, assignment)
        )
...
def variable_elimination(
    factors, query, evidence, hidden
):
    # Apply evidence
    for ev, val in evidence.items():
        factors = [
            restrict(f, ev, val)
            if ev in f.variables else f
            for f in factors
        ]
